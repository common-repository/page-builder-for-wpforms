...
class CurrencyMultipleOptionsFieldSettings extends FieldSettingsBase
{
    ...
    public function AddOption($label,$value,$price=''){
        $this->Items[]=new CurrencyMultipleOptionsItem($label,$value!=''?$value:$label,$price);
    }
    ...
    public function InitializeFromOptions($options)
    {
        ...
        foreach($options->Items as $Item)
        {
            ...
            if(isset($Item->Price))
                $price=$Item->Price;
            $this->Items[]=new CurrencyMultipleOptionsItem($Item->Label,$Item->Value,$price);
        }
    }
    ...
    {
        $value= esc_html(parent::ParseValue($value, $pathId));
        $value=nl2br($value);
        if($pathId=='Value')
            return html_entity_decode($value);
...
class CurrencyMultipleOptionsItem{
    public $Label;
    public $Value;
    public $Price;
