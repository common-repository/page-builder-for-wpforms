<?php
/**
 * Created by PhpStorm.
 * User: Edgar
 * Date: 3/28/2019
 * Time: 4:26 AM
 */

namespace rnpagebuilder\core\Integration\Adapters\WPForm\Settings\Forms;


use Exception;
use rnpagebuilder\core\Integration\Adapters\WPForm\Settings\Forms\Fields\WPFormAddressFieldSettings;
use rnpagebuilder\core\Integration\Adapters\WPForm\Settings\Forms\Fields\WPFormDateFieldSettings;
use rnpagebuilder\core\Integration\Adapters\WPForm\Settings\Forms\Fields\WPFormNameFieldSettings;
use rnpagebuilder\core\Integration\Processors\Settings\Forms\Fields\CurrencyFieldSettings;
use rnpagebuilder\core\Integration\Processors\Settings\Forms\Fields\CurrencyMultipleOptionsFieldSettings;
use rnpagebuilder\core\Integration\Processors\Settings\Forms\Fields\FieldSettingsBase;
use rnpagebuilder\core\Integration\Processors\Settings\Forms\Fields\HtmlFieldSettings;
use rnpagebuilder\core\Integration\Processors\Settings\Forms\Fields\LikertScaleFieldSettings;
use rnpagebuilder\core\Integration\Processors\Settings\Forms\FieldSettingsFactoryBase;

class WPFormFieldSettingsFactory extends FieldSettingsFactoryBase
{
    /**
     * @param $options
     * @return FieldSettingsBase
     * @throws Exception
     */
    public function GetFieldByOptions($options)
    {
        $field= parent::GetFieldByOptions($options);
        if($field!=null)
            return $field;

        switch ($options->Type)
        {
            case 'Address':
                $field=new WPFormAddressFieldSettings();
                break;
            case 'Date':
                $field=new WPFormDateFieldSettings();
                break;
            case 'Name':
                $field=new WPFormNameFieldSettings();
                break;
            case "HTML":
                $field=new HtmlFieldSettings();
                break;
            case 'Currency':
                $field=new CurrencyFieldSettings();
                break;
            case 'CurrencyMultiple':
                $field=new CurrencyMultipleOptionsFieldSettings();
                break;
            case 'LikertScale':
                $field=new LikertScaleFieldSettings();
                break;
        }

        if($field==null)
            throw new Exception('Invalid field settings type '.$options->Type);

        $field->InitializeFromOptions($options);
        return $field;
    }


}