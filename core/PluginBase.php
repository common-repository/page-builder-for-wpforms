...
use rnpagebuilder\ajax\RunnableAjax;
...
use rnpagebuilder\PageBuilderOld\PostItem;
...
abstract class PluginBase
{
    ...
    public function __construct($prefix,$dbVersion,$fileVersion,$rootFilePath,$config=null)
    {
        ...
        add_shortcode('rnpbpreview',array($this,'Preview'));
        add_shortcode('pbpage',array($this,'GeneratePage'));


        new RunnableAjax($this);
    }
    ...
    public function GeneratePage($attr,$content){
        return RNPB()->ShowPage(intval($content));
    }

    public function Preview(){

        $pageBuilder=new PageBuilderAjax($this);

        $preview= $pageBuilder->Preview();
